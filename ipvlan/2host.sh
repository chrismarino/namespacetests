...
echo 'Add the routes...'
ip netns exec nspace0 ip route add default via $GW_PHY dev ns0
ip netns exec nspace1 ip route add default via $GW_PHY dev ns1
ip netns exec nspace2 ip route add default via $GW_PHY dev ns2
ip netns exec nspace3 ip route add default via $GW_PHY dev ns3
