...
#Namespace gateways are the host
GW_NS0=$ADDR_ETH0
GW_NS1=$ADDR_ETH0
#GW_NSR=$ADDR_VETHR
...
# Create the veth pair that links the host to the router.
echo 'Create the veth pairs'
ip link add nsR type veth peer name vethR

# Now create the IPVLAN links...
echo 'Create the IPVLANs links...'
ip link add link nsR ns0 type ipvlan mode $MODE
ip link add link nsR ns1 type ipvlan mode $MODE
ip link add link nsR nsR type ipvlan mode $MODE
